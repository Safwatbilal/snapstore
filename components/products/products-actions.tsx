...
const ProductsActions: React.FC<{ id: string }> = ({ id }) => {
  ...
  const { control, handleSubmit, watch, reset } = useForm<IProductAction>({
    defaultValues: defaultProductAction,
    resolver: yupResolver(
      productValidation(),
    ) as unknown as Resolver<IProductAction>,
  ...
  return (
    <Sheets
      ...
      description={t("global.add_description")}
    >
        ...
        <div className="form">
          ...
          <div className="divButtonForm">
            <Button
              ...
              onClick={() =>
                dispatch(
                  updateControlState({ key: "openSheet", payload: false }),
                )
              }
            >
              {t("global.back")}
