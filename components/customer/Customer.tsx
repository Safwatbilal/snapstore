...
const Customer = () => {
  ...
  const { data: myCustomer, isLoading } = queries.getAllOrdersToOwner(
    userId as string,
    state,
  );
  ...
  return (
    <>
        ...
        {myCustomer?.map((order) =>
          order.cartArray.map((customer) => (
            <TableRow key={rowIndex}>
              ...
              <TableCell>
                <StateBedget state={customer.state}>
                  {customer.state}
                </StateBedget>
              </TableCell>
                  ...
                  <DialogTrigger asChild>
                    <TooltipButton
                      ...
                      icon={
                        ...
                        >
                          <Info size={20} className="dark:text-white"></Info>
                        </Badge>
                      }
                    />
                  </DialogTrigger>
      ...
      {!isLoading && myCustomer && myCustomer[0]?.cartArray.length === 0 && (
        <NoData title="no customer" />
      )}
    </>
  );
};
